...
```{r comments-over-time-plot, echo=T, eval=F, dpi=300, out.width="700px", out.height="500px"}
weekly_comments %>% 
  ggplot(aes(x = week, y = n)) +
  geom_bar(stat = "identity") +
  scale_x_date(expand = c(0,0)) +
...
.smaller[
```{r comments-likes-output, echo = T, eval=F}
FormattedComments %>% 
  arrange(-LikeCount) %>% 
  head(10) %>% 
  select(Text, LikeCount, Published)
...
```{r cloudy-plot, echo = F, eval=F, dpi=300, out.width="500px", out.height="500px"}
wordcloud(words = TermFreq$feature,
          freq = TermFreq$frequency,
          min.freq = 10,
          max.words = 50,
...
```{r word-freq-plot, echo=F, eval=F, dpi=300, out.width="700px", out.height="500px"}
TermFreq %>% 
head(n = 20) %>% 
  ggplot(aes(x = reorder(feature, frequency), y = frequency)) +
  geom_bar(stat="identity") +
...
```{r docfreq-plot, eval=F, echo=F, dpi=300, out.width="700px", out.height="500px"}
TermFreq %>% 
head(n = 20) %>% 
  ggplot(aes(x = reorder(feature, docfreq), y = docfreq)) +
  geom_bar(stat="identity") + 
...
```{r emoji-barplot, echo=F, eval=F, dpi=300, out.width="700px", out.height="500px"}
EmojiFreq %>% 
head(n = 10) %>% 
  ggplot(aes(x = reorder(feature, frequency), y = frequency)) +
  geom_bar(stat="identity") + 
...
```{r cool-emoji-plot, echo=F, eval=F, dpi=300, out.width="700px", out.height="500px"}
EmojiFreq %>% 
head(n = 10) %>% 
  ggplot(aes(x = reorder(feature, -frequency), y = frequency)) +
  geom_bar(stat="identity",
...
## [Exercise](https://jobreu.github.io/youtube-workshop-gesis-2022/exercises/B1_Basic_text_analysis_exercises_question.html) time `r ji("weight_lifting_woman")``r ji("muscle")``r ji("running_man")``r ji("biking_man")`
